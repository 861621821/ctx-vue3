<template>
  <div class="account-container">
    <el-tabs :model-value="Object.keys(data)[0]" @tab-click="handleClick">
      <el-tab-pane
        :label="platform"
        :name="platform"
        v-for="(envs, platform) in data"
        :key="platform"
      >
        <el-scrollbar>
          <div class="platform">
            <div v-for="(list, env) in envs" :key="env">
              <div class="vibe-title">
                {{ env
                  ...
                  @click.stop="handleAdd(list, platform, env)"
                  class="iconfont icon-tianjiachengyuan"
                ></i>
              </div>
              <div class="vibe-data">
                <el-table
                  :data="list"
                  @row-dblclick="handleLogin"
                  style="
                  ...
                  <el-table-column>
                    <template #default="{ row }">
                      <el-input
                        ...
                        :class="{
                          readonly: !row.isEdit,
                          'is-error': row.accountError && row.isEdit,
                        }"
                        :readonly="!row.isEdit"
                        v-model="row.account"
                      ></el-input>
                  ...
                  <el-table-column>
                    <template #default="{ row }">
                      <el-input
                        ...
                        :class="{
                          readonly: !row.isEdit,
                          'is-error': row.pwdError && row.isEdit,
                        }"
                        :readonly="!row.isEdit"
                        v-model="row.pwd"
                      ></el-input>
  ...
  if (row.id) {
    ...
    if (row.account && row.pwd) {
      try {
        ...
        ElMessage(typeof error === "string" ? error : error.message);
      }
    }
  }
};
...
onMounted(async () => {
  chrome.storage.local.get("account", (res) => {
    const account = res.account
      ? JSON.parse(JSON.stringify(res.account))
      : undefined;
    if (account) {
